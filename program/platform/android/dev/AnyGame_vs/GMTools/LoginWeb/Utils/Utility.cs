...
namespace LoginWeb
{
    public class Utility
    {
        ...
        public static string ReceiveResponse(string url, string data, Encoding encode, int timeOutSeconds = 10)
        {
            ...
            {
                Logs.Error(string.Format("call {0} fail.", url), ex);
                return string.Empty;
            }
        }
        /// <summary>
        ...
        public static Hashtable ConvertKeyValueStrToHashtableExt(string keyValueStr, char connector = '&', char spilt = '=')
        {
            Hashtable ht = null;
            if (!string.IsNullOrEmpty(keyValueStr) && keyValueStr.IndexOf(connector.ToString()) > -1 && keyValueStr.IndexOf(connector.ToString()) > -1)
            {
                ...
                foreach (string keyValue in keyValueStr.Split(connector))
                {
                    var index = keyValue.IndexOf(spilt);
                    if (index >= 0)
                    {
                        var k = keyValue.Substring(0, index);
                        var v = keyValue.Substring(index + 1);
