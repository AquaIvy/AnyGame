...
using System;
using System.IO;
using DogSE.Client.Core.Util;

namespace DogSE.Client.Core.Config
{
    /// <summary>
    /// 动态配置文件管理
    ...
    public static class DynamicConfigFileManager
    {
        ...
        public static void LoadData(string folder = null)
        {
        ...
        private static void LoadCSVConfig(string folder = null)
        {
            ...
            foreach (var type in configTypes)
            {
                ...
                if (folder == null)
                    folder = AppDomain.CurrentDomain.BaseDirectory;
                var fileName = Path.GetFullPath(Path.Combine(folder, rootAttribute.FileName));
