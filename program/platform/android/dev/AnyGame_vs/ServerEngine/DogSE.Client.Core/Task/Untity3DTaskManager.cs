...
namespace DogSE.Client.Core.Task
{
    ...
    /// </remarks>
    public class Unity3DTaskManager
    {
        ...
        public Unity3DTaskManager(string taskName)
        {
            taskName_ = taskName;
        }
        ...
        {
            ITask task;
            while (taskList.TryDequeue(out task))
            {
                //watch.Restart();
                taskWatch.Reset();
                taskWatch.Start();
                ...
                task.TaskProfile.Append(taskWatch.ElapsedTicks, isError);
                task.WriteLog(taskWatch.ElapsedTicks, isError);
                task.Release();
            }
        }
